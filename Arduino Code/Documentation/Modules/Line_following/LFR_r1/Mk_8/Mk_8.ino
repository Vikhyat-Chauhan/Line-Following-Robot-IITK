...
void linefollowing(){
  ...
  if(digitalRead(junctionPulse)) { 
    sendCommand('D',1);
    byte positionVal = 0; 
    for(int i=0;i<3;i++){  // Declare a psition variable to store incomin
    while(mySerial.available() <= 0);   // Wait for data to be available
    positionVal = mySerial.read();    // Read incoming data and store in dummy
    }
    while(mySerial.available() <= 0);   // Wait for data to be available
    ...
    if((positionVal==248 || positionVal==240 || positionVal==224)){  //RIGHT 252
          digitalWrite(3,LOW);
          analogWrite(5,base_speed);
          digitalWrite(6,LOW);
          analogWrite(9,base_speed);
        }
    else if((positionVal==7 || positionVal==15 || positionVal==31)){  //LEFT
          analogWrite(3,base_speed);
          digitalWrite(5,LOW);
          analogWrite(6,base_speed);
          digitalWrite(9,LOW);
        }
    delay(300);
    sendCommand('D',2);
