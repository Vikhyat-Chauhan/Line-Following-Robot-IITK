...
SoftwareSerial wifiSerial(18,19); // RX, TX
...
void linefollowing(){
  ...
  if(digitalRead(junctionPulse)) { 
    sendCommand('D',1);
      int positionVal;   // Declare a psition variable to store incomin
      while(mySerial.available() <= 0);   // Wait for data to be available
      positionVal = mySerial.read();    // Read incoming data and store in dummy
      Serial.println(positionVal);
      analogWrite(6,0);
      analogWrite(5,0);delay(1000);
     if((positionVal==7 || positionVal==15 || positionVal==31) && (positionVal!=248 || positionVal!=240 || positionVal!=224)){
       analogWrite(6,0);
       analogWrite(5,150);
       delay(300);
     }
     if((positionVal==248 || positionVal==240 || positionVal==224)&&(positionVal!=7 || positionVal!=15 || positionVal!=31)){
       analogWrite(6,150);
       analogWrite(5,0);
       delay(300);
     }
